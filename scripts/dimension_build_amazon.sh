	...
	if [[ "${OS_NAME}" == "Amazon Linux AMI" && "${OS_VER}" -lt 2017 ]]; then
		printf "\\tYou must be running Amazon Linux 2017.09 or higher to install Dimension.\\n"
		printf "\\texiting now.\\n"
		exit 1
	fi

	if [ "${DISK_AVAIL}" -lt "${DISK_MIN}" ]; then
		printf "\\tYou must have at least %sGB of available storage to install Dimension.\\n" "${DISK_MIN}"
		printf "\\texiting now.\\n"
		exit 1
	fi
	...
	then
		printf "\\n\\tYum must be installed to compile Dimension.\\n"
		printf "\\n\\tExiting now.\\n"
		exit 1
	fi
