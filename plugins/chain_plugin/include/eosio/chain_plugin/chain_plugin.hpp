...
public:
   ...
   get_proposals_result get_proposals( const get_proposals_params& params )const;

   struct get_gnode_params {
      bool        json = false;
      string      lower_bound;
      uint32_t    limit = 50;
   };

   struct get_gnode_result {
      vector<fc::variant> rows; ///< one row per item, either encoded as hex string or JSON object
      string              more; ///< fill lower_bound with this value to fetch more rows
   };

   get_gnode_result get_gnode( const get_gnode_params& params )const;
...
FC_REFLECT( eosio::chain_apis::read_only::get_proposals_params, (json)(lower_bound)(limit) )
FC_REFLECT( eosio::chain_apis::read_only::get_proposals_result, (rows)(more) );

FC_REFLECT( eosio::chain_apis::read_only::get_gnode_params, (json)(lower_bound)(limit) )
FC_REFLECT( eosio::chain_apis::read_only::get_gnode_result, (rows)(more) );
