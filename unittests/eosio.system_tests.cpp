...
BOOST_FIXTURE_TEST_CASE( buysell, eosio_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), stake( "eonio", "alice1111111", core_from_string("200.0000"), core_from_string("100.0000") ) );
   ...
   BOOST_REQUIRE_EQUAL( initial_ram_balance + core_from_string("199.0000"), get_balance(N(eonio.ram)) );
   BOOST_REQUIRE_EQUAL( initial_ramfee_balance + core_from_string("1.0000"), get_balance(N(eonio.ramfee)) );
   ...
   transfer( "eonio", "alice1111111", core_from_string("100000000.0000"), "eonio" );
   ...
   BOOST_REQUIRE_EQUAL( core_from_string("99396507.4139"), get_balance( "alice1111111" ) );
...
BOOST_FIXTURE_TEST_CASE( stake_unstake, eosio_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( init_eosio_stake_balance + core_from_string("300.0000"), get_balance( N(eonio.stake) ) );
...
BOOST_FIXTURE_TEST_CASE( stake_unstake_with_transfer, eosio_system_tester ) try {
   ...
   issue( "eonio", core_from_string("1000.0000"), config::system_account_name );
   issue( "eonio.stake", core_from_string("1000.0000"), config::system_account_name );
   BOOST_REQUIRE_EQUAL( core_from_string("0.0000"), get_balance( "alice1111111" ) );

   //eonio stakes for alice with transfer flag

   transfer( "eonio", "bob111111111", core_from_string("1000.0000"), "eonio" );
   ...
   transfer( "eonio", "alice1111111", core_from_string("1000.0000"), "eonio" );
   BOOST_REQUIRE_EQUAL( success(), stake( "alice1111111", "alice1111111", core_from_string("200.0000"), core_from_string("100.0000") ) );
   //now alice's stake should be equal to transfered from eonio + own stake
   total = get_total_stake("alice1111111");
   BOOST_REQUIRE_EQUAL( core_from_string("700.0000"), get_balance( "alice1111111" ) );
...
BOOST_FIXTURE_TEST_CASE( fail_without_auth, eosio_system_tester ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), stake( "eonio", "alice1111111", core_from_string("2000.0000"), core_from_string("1000.0000") ) );
...
BOOST_FIXTURE_TEST_CASE( producer_register_unregister, eosio_system_tester ) try {
   ...
   auto key =  fc::crypto::public_key( std::string("EON6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV") );
   ...
   auto key2 =  fc::crypto::public_key( std::string("EON5jnmSKrzdBHE9n8hw58y7yxFWBC8SNiG7m8S1crJH3KvAnf9o6") );
   BOOST_REQUIRE_EQUAL( success(), push_action(N(alice1111111), N(regproducer), mvo()
                                               ("producer",  "alice1111111")
                                               ("producer_key", key2 )
                                               ("url", "http://block.two")
...
BOOST_FIXTURE_TEST_CASE( vote_for_producer, eosio_system_tester, * boost::unit_test::tolerance(1e+5) ) try {
   ...
   BOOST_REQUIRE_EQUAL( success(), unstake( "carol1111111", core_from_string("2.0000"), core_from_string("0.0002")/*"2.0000 EON", "0.0002 EON"*/ ) );
...
BOOST_FIXTURE_TEST_CASE(multiple_producer_pay, eosio_system_tester, * boost::unit_test::tolerance(1e-10)) try {
...

   {
      ...
      BOOST_REQUIRE_EQUAL( error("missing authority of eonio"),
...
BOOST_FIXTURE_TEST_CASE(producer_onblock_check, eosio_system_tester) try {
   ...
   {
      const char* claimrewards_activation_error_message = "cannot undelegate bandwidth until the chain is activated (at least 1 000 000)";
...
BOOST_FIXTURE_TEST_CASE( elect_producers /*_and_parameters*/, eosio_system_tester ) try {
   ...
   //stake more than 15% of total EON supply to activate chain
   transfer( "eonio", "alice1111111", core_from_string("600000000.0000"), "eonio" );
...
BOOST_FIXTURE_TEST_CASE( buyname, eosio_system_tester ) try {
   ...
   transfer( "eonio", "nofail", core_from_string( "1000.0000" ) );
...
BOOST_FIXTURE_TEST_CASE( multiple_namebids, eosio_system_tester ) try {
   ...
   // alice outbids bob on prefb
   {
      ...
      BOOST_REQUIRE_EQUAL( initial_names_balance + core_from_string("0.1001"), get_balance(N(eonio.names)) );
   }
