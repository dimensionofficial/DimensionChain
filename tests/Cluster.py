...
class Cluster(object):
    ...
    def launch(self, pnodes=1, totalNodes=1, prodCount=1, topo="mesh", p2pPlugin="net", delay=1, onlyBios=False, dontBootstrap=False,
        ...
        extraNodeosArgs: string of arguments to pass through to each nod instance (via --nodeon flag on launcher)
        ...
        specificExtraNodeosArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-nodeon flags on launcher), example: { "5" : "--plugin eosio::test_control_api_plugin" }
        """
        ...
        while not Utils.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on nodeon default port. wait...")
            if tries == 0:
                return False
            tries = tries - 1
            time.sleep(2)
    ...
    def bios_bootstrap(totalNodes, biosHost, biosPort, walletMgr, silent=False):
        ...
        for name, keys in producerKeys.items():
            data="{\"from\":\"eonio\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init transfer")
