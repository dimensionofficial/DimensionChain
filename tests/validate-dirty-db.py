...
###############################################################
# Test for validating the dirty db flag sticks repeated nodeon restart attempts
###############################################################
...
def runNodeosAndGetOutput(myTimeout=3):
    """Startup nodeon, wait for timeout (before forced shutdown) and collect output. Stdout, stderr and return code are returned in a dictionary."""
    Print("Launching nodeon process.")
    ...
    output={}
    try:
        if debug: Print("Setting nodeon process timeout.")
        ...
        Print("ERROR: Nodeos is running beyond the defined wait time. Hard killing nodeon instance.")
...
try:
    ...
    Print("Restart nodeon repeatedly to ensure dirty database flag sticks.")
    timeout=6
    ...
    for i in range(1,4):
        ...
        if not ret[0]:
            errorExit("Failed to startup nodeo sucessfully on try number %d" % (i))
        assert(ret[1])
