...
The transaction to create the `eonio.sudo` account will need to be proposed to get the necessary approvals from active block producers before executing it. This transaction needs to first be generated and stored as JSON into a file so that it can be used in the cleos command to propose the transaction to the eonio.msig contract.
...
```
$ cleos system newaccount -s -j -d --transfer --stake-net "1.000 EON" --stake-cpu "1.000 EON" --buy-ram-kbytes 50 eonio eonio.sudo EOS8MMUW11TAdTDxqdSwSqJodefSoZbFhcprndomgLi9MeR2o8MT4 > generated_account_creation_trx.json
...
```
$ cleos push action -s -j -d eonio setpriv '{"account": "eonio.sudo", "is_priv": 1}' -p eonio > generated_setpriv_trx.json
...
```
$ cleos set contract -s -j -d eonio.sudo contracts/eonio.sudo/ > deploy_sudo_contract_trx.json
...
```
$ cleos get code -a retrieved-eonio.sudo.abi eonio.sudo
...
```
$ cleos push action -s -j -d eonio.sudo exec '{"executer": "blkproducera", "trx": ""}' > sudo_update_alice_owner_trx.json
$ cat sudo_update_alice_owner_trx.json
{
