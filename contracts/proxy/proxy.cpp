...
namespace proxy {
   ...
   void apply_transfer(uint64_t receiver, account_name /* code */, const T& transfer) {
      ...
      if (transfer.from == self) {
         ...
         transaction out;
         out.actions.emplace_back(permission_level{self, N(active)}, N(eonio.token), N(transfer), new_transfer);
...
extern "C" {
    ...
    void apply( uint64_t receiver, uint64_t code, uint64_t action ) {
      if( code == N(eonio) && action == N(onerror) ) {
         apply_onerror( receiver, onerror::from_current_action() );
      } else if( code == N(eonio.token) ) {
