...
namespace eosiosystem {
   ...
   void system_contract::buyram( account_name payer, account_name receiver, asset quant )
   {
      ...
      INLINE_ACTION_SENDER(eosio::token, transfer)( N(eonio.token), {payer,N(active)},
         { payer, N(eonio.ram), quant_after_fee, std::string("buy ram") } );

      if( fee.amount > 0 ) {
         INLINE_ACTION_SENDER(eosio::token, transfer)( N(eonio.token), {payer,N(active)},
                                                       { payer, N(eonio.ramfee), fee, std::string("ram fee") } );
      }
   ...
   void system_contract::sellram( account_name account, int64_t bytes ) {
      ...
      INLINE_ACTION_SENDER(eosio::token, transfer)( N(eonio.token), {N(eonio.ram),N(active)},
                                                       { N(eonio.ram), account, asset(tokens_out), std::string("sell ram") } );

      auto fee = ( tokens_out.amount + 199 ) / 200; /// .5% fee (round up)
      // since tokens_out.amount was asserted to be at least 2 earlier, fee.amount < tokens_out.amount
      
      if( fee > 0 ) {
         INLINE_ACTION_SENDER(eosio::token, transfer)( N(eonio.token), {account,N(active)},
            { account, N(eonio.ramfee), asset(fee), std::string("sell ram fee") } );
      }
   }
                                   ...
                                   const asset stake_net_delta, const asset stake_cpu_delta, bool transfer )
   {
      ...
      if ( N(eonio.stake) != source_stake_from ) { //for eosio both transfer and refund make no sense
         ...
         if ( asset(0) < transfer_amount ) {
            INLINE_ACTION_SENDER(eosio::token, transfer)( N(eonio.token), {source_stake_from, N(active)},
               { source_stake_from, N(eonio.stake), asset(transfer_amount), std::string("stake bandwidth") } );
         }
      }
   ...
   void system_contract::refund( const account_name owner ) {
      ...
      INLINE_ACTION_SENDER(eosio::token, transfer)( N(eonio.token), {N(eonio.stake),N(active)},
                                                    { N(eonio.stake), req->owner, req->net_amount + req->cpu_amount, std::string("unstake") } );
