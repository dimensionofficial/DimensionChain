...
namespace eosiosystem {
    ...
    _rammarket(_self,_self)
   {
      ...
      if( itr == _rammarket.end() ) {
         auto system_token_supply   = eosio::token(N(eonio.token)).get_supply(eosio::symbol_type(system_token_symbol).name()).amount;
   ...
   void system_contract::bidname( account_name bidder, account_name newname, asset bid ) {
      ...
      INLINE_ACTION_SENDER(eosio::token, transfer)( N(eonio.token), {bidder,N(active)},
                                                    { bidder, N(eonio.names), bid, std::string("bid name ")+(name{newname}).to_string()  } );
      ...
      if( current == bids.end() ) {
         ...
         INLINE_ACTION_SENDER(eosio::token, transfer)( N(eonio.token), {N(eonio.names),N(active)},
                                                       { N(eonio.names), current->high_bidder, asset(current->high_bid),
